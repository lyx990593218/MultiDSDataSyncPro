 ...
 * 
 * @author Fandy Liu
 * @created 2015年6月1日 下午6:21:26
 */
public class SimpleLunarCalendar {

    /** 最小时间1900-1-31 */
    private final static long MIN_TIME_IN_MILLIS = -2206425952001L;
    /** 最大时间2099-12-31 */
    private final static long MAX_TIME_IN_MILLIS = 4102416000000L;
    /**
    ...
    /** 农历中文字符串一 */
    private final static String[] LUNAR_STRING_1 = { "零", "一", "二", "三", "四", "五", "六", "七", "八", "九" };
    /** 农历中文字符串二 */
    private final static String[] LUNAR_STRING_2 = { "初", "十", "廿", "卅", "正", "腊", "冬", "闰" };
    /** 农历年 */
    ...
    private void init(long timeInMillis) {
        if (timeInMillis > MIN_TIME_IN_MILLIS && timeInMillis < MAX_TIME_IN_MILLIS) {
    ...
    private static int getLunarYearDays(int lunarYear) {
        ...
        for (int i = 0x8000; i > 0x8; i >>= 1) {
            // 每个大月累加一天
            daysInLunarYear += ((SimpleLunarCalendar.LUNAR_INFO[lunarYear - 1900] & i) != 0) ? 1 : 0;
        }
        // 加上闰月天数
    ...
    private static int getLunarLeapDays(int lunarYear) {
        ...
        return SimpleLunarCalendar.getLunarLeapMonth(lunarYear) > 0
                ? ((SimpleLunarCalendar.LUNAR_INFO[lunarYear - 1899] & 0xf) == 0xf ? 30 : 29) : 0;
    }
    ...
    private static int getLunarLeapMonth(int lunarYear) {
        // 匹配后4位
        int leapMonth = SimpleLunarCalendar.LUNAR_INFO[lunarYear - 1900] & 0xf;
        // 若最后4位全为1或全为0,表示没闰
    ...
    private static int getLunarMonthDays(int lunarYear, int lunarMonth) {
        ...
        int daysInLunarMonth = ((SimpleLunarCalendar.LUNAR_INFO[lunarYear - 1900] & (0x10000 >> lunarMonth)) != 0) ? 30
                : 29;
    ...
    private static String getLunarYearString(int lunarYear) {
        ...
        StringBuffer lunarYearSb = new StringBuffer();
        String year = String.valueOf(lunarYear);
        for (int i = 0; i < year.length(); i++) {
            char yearChar = year.charAt(i);
            int index = Integer.parseInt(String.valueOf(yearChar));
            // lunarYearString += LUNAR_STRING_1[index];
            lunarYearSb.append(LUNAR_STRING_1[index]);
        }
        lunarYearString = lunarYearSb.toString();
    ...
    private static String getLunarMonthString(int lunarMonth) {
        ...
        if (lunarMonth == 1) {
            lunarMonthString = SimpleLunarCalendar.LUNAR_STRING_2[4];
        } else {
            if (lunarMonth > 9)
                lunarMonthString += SimpleLunarCalendar.LUNAR_STRING_2[1];
            if (lunarMonth % 10 > 0)
                lunarMonthString += SimpleLunarCalendar.LUNAR_STRING_1[lunarMonth % 10];
        }
    ...
    private static String getLunarDayString(int lunarDay) {
        ...
        int i1 = lunarDay / 10;
        int i2 = lunarDay % 10;
        String c1 = SimpleLunarCalendar.LUNAR_STRING_2[i1];
        String c2 = SimpleLunarCalendar.LUNAR_STRING_1[i2];
        if (lunarDay < 11)
            c1 = SimpleLunarCalendar.LUNAR_STRING_2[0];
        if (i2 == 0)
            c2 = SimpleLunarCalendar.LUNAR_STRING_2[1];
        return c1 + c2;
    }
