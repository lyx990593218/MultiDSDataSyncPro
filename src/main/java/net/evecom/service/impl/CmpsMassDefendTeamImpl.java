...
public class CmpsMassDefendTeamImpl extends BaseServiceImpl implements IDataSync {
    ...
    @Value("${daysRangeEdge.massDefendTeam}")
    private String daysRangeEdge;

    @Value("${sysDebug.massDefendTeam}")
    ...
    public String getFindTargetSql() {
        return "SELECT T.* FROM (SELECT TT.*, ROW_NUMBER() OVER (PARTITION BY TT.CERT_NUM ORDER BY TT.UPLOAD_TIME DESC) RN FROM PINGTAN."  + TABLENAME + " TT WHERE CERT_NUM = ?) T WHERE 1 = 1 AND T.RN = 1";
    }
