...
public class CmpsMassDefendOrgImpl extends BaseServiceImpl implements IDataSync {
    ...
    @Value("${daysRangeEdge.massDefendOrg}")
    private String daysRangeEdge;

    @Value("${sysDebug.massDefendOrg}")
    ...
    public String getFindTargetSql() {
        return "SELECT T.* FROM (SELECT TT.*, ROW_NUMBER() OVER (PARTITION BY TT.MASS_ORG_NAME ORDER BY TT.UPLOAD_TIME DESC) RN FROM PINGTAN."  + TABLENAME + " TT WHERE MASS_ORG_NAME = ?) T WHERE 1 = 1 AND T.RN = 1";
    }
